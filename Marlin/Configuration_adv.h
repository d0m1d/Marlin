 ...
 */
  ...
  // Better lower time, but higher hysteresis, thus the failed heater has less time to reach critical temps.
...
#define USE_CONTROLLER_FAN
  ...
  #define CONTROLLER_FAN_PIN MOSFET_D_PIN // labeled as E1, Set a custom pin for the controller fan
 ...
 */
#define E0_AUTO_FAN_PIN 51  // dummy pin, hotend fan connected to +12V, but PSU needs to stay on till temp <40°C (somehow X_MAX_PIN and Z_MAX_PIN didn't work)
...
//#define CODEPENDENT_XY_HOMING
  ...
  #undef BLTOUCH_DELAY  // bec. it is first defined (with ifndef) in Conditionals_LCD.h
...
// @section lcd
  ...
  #define MANUAL_FEEDRATE {80*60, 80*60, 7*60, 4*60} // Feedrates for manual moves along X, Y, Z, E from panel
...
#define ENCODER_100X_STEPS_PER_SEC 1000  // If the encoder steps per sec exceeds this value, multiply steps moved x100 to really quickly advance the value
  //100x set so high to deactivate it
...
// The timeout (in ms) to return to the status screen from sub-menus, undefined is 15s
...
#define ADVANCED_PAUSE_FEATURE
  ...
  #define FILAMENT_UNLOAD_RETRACT_LENGTH       7  // (mm) Unload initial retract length.
  #define FILAMENT_UNLOAD_DELAY             5000  // (ms) Delay for the filament to cool after retract.
  #define FILAMENT_UNLOAD_PURGE_LENGTH         5  // (mm) An unretract is done, then this length is purged.

  #define PAUSE_PARK_NOZZLE_TIMEOUT           10  // (seconds) Time limit before the nozzle is turned off for safety. So short bec hotend fan failure also triggers runout.
