...
public:
  ...
  static constexpr uint8_t ocr_to_pct(const uint8_t ocr) { return uint8_t(PWM_TO_PCT(ocr)); }

  // cpower = configured values (e.g., SPEED_POWER_MAX)
  // Convert configured power range to a percentage
  static constexpr cutter_cpower_t power_floor = TERN(CUTTER_POWER_RELATIVE, SPEED_POWER_MIN, 0);
  static constexpr cutter_cpower_t power_range = SPEED_POWER_MAX - power_floor;
  ...
  static constexpr uint8_t cpwr_to_ocr(const cutter_cpower_t cpwr) {
    return cpwr ? round(255.0f * (cpwr - power_floor) / power_range) : 0;
  }
  // Convert ocr (PWM255) to configured power range
  static constexpr cutter_power_t ocr_to_cpwr(const uint8_t ocr) {
  ...
  static constexpr cutter_power_t cpwr_to_upwr(const cutter_cpower_t cpwr) { // STARTUP power to Unit power
    return (
        ...
        #else
          cpwr_to_ocr(cpwr)               // to PWM // PCT_TO_PWM(cpwr_to_pct(cpwr)) // thus you lose 2.55 times resolution
        #endif
      #else
