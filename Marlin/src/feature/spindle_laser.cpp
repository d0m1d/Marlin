...
void SpindleLaser::apply_power(const uint8_t opwr) {
  if (enabled() /*|| opwr == 0*/) {                                   // 0 check allows us to disable where no ENA pin exists // sonst enable pin immer ON
    ...
    // Handle PWM driven or just simple on/off
      ...
      else if (/*ENABLED(CUTTER_POWER_RELATIVE) ||*/ enabled() /*|| opwr == 0*/) { // sonst enable pin immer ON
        set_ocr(opwr);
        isReadyForUI = true;
      }
      else {
